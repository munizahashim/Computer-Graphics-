...
void draw_line_x1(double x1, double y1, double x2, double y2) {
   ...
   double m, y;
   m = (y2 - y1) / (x2 - x1);
   y = y1 + m*(round(x1) + .5 - x1);
   x = round(x1);
   while (x < round(x2)) {
      iy = round(y);

	set_pixel(x, iy, 1, 1, 1);
    //set_pixel(x, iy, 1, 1, 1);
    //set_pixel(x, iy+1, 1, 1, 1);

	//set_pixel(x, iy, 255, 255, 255);
    //set_pixel(x, iy+1, 255, 255, 255);
      y += m;
      x++;
   }
}
...
void draw_line_y1(double x1, double y1, double x2, double y2) {
   ...
   double m, x;
   m = (x2 - x1) / (y2 - y1);
   x = x1 + m*(round(y1) + .5 - y1);
   y = round(y1);
   while (y < round(y2)) {
      ix = round(x);
    set_pixel(ix, y, 1, 1, 1);
	//set_pixel(ix, y, 1, 1, 1);
    //set_pixel(ix, y+1, 1, 1, 1);

	//set_pixel(ix, y, 255, 255, 255);
    //set_pixel(ix, y+1, 255, 255, 255);
      x += m;
      y++;
   }
}
...
int main(int argc, char** argv) {  
   ...
   for (j = 0; j <= n; ++j) {
	...
	write_bmp((char*) "AntialiseLine_x.bmp", (char*) image, image_size, width);
	return 1;
}